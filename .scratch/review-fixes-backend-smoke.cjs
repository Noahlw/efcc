...
const users = [
  ["User_ID", "Username", "PIN", "Name", "Role", "Status", "QR_Code_String", "Phone"],
  ["USER-STAFF", "staff", "1111", "Staffer", "STAFF", "Active", "QR-STAFF", ""],
  ["USER-MEMBER-A", "membera", "2222", "Member A", "MEMBER", "Active", "QR-A", ""],
  ["USER-MEMBER-B", "memberb", "3333", "Member B", "MEMBER", "Active", "QR-B", ""],
];
const enrollments = [
...
const events = [
  ["Event_ID", "Program_ID", "Event_Name", "Event_Date", "Time_Slot", "Event_Type", "Recurrence_Type", "Status", "Created_By", "Created_At"],
  ["EVT-1", "PROG-1", "Sunday Service", "01/08/2026", "10:00", "REGULAR", "NONE", "Active", "USER-STAFF", "2026-01-01"],
  ["EVT-2", "PROG-1", "Special Event", "02/08/2026", "10:00", "SPECIAL", "NONE", "Active", "USER-STAFF", "2026-01-01"],
  ["EVT-CANCELLED", "PROG-1", "Cancelled Event", "03/08/2026", "10:00", "REGULAR", "NONE", "Cancelled", "USER-STAFF", "2026-01-01"],
];
const attendance = [
  ["Attendance_ID", "Event_ID", "User_ID", "Check_In_Time", "Check_In_Method", "Check_In_By", "Status"],
];
...
if (!cancelRes.success) throw new Error("cancelEvent should succeed: " + cancelRes.message);
// EVT-1 is events[1] (row 2 of sheet incl header). Its status column must flip, NOT EVT-2's.
if (events[1][7] !== "Cancelled") throw new Error("api_cancelEvent did not cancel the TARGET row (off-by-one regression)");
if (events[2][7] !== "Active") throw new Error("api_cancelEvent corrupted the WRONG row (off-by-one bug present)");
...
if (context.verifySessionToken_("USER-MEMBER-A", memberAToken)) {
  throw new Error("verifySessionToken_ accepted a token with no issuedAt record");
}
...
if (context.verifySessionToken_("USER-MEMBER-A", memberAToken)) {
  throw new Error("verifySessionToken_ accepted a token after logout (revocation not enforced)");
}
...
if (notEnrolledRes.success !== false || notEnrolledRes.notEnrolled !== true) {
  throw new Error("api_checkInMember did not flag an unenrolled member: " + JSON.stringify(notEnrolledRes));
}
if (notEnrolledRes.data.memberName !== "Member B") {
  throw new Error("api_checkInMember did not resolve member name for notEnrolled response");
}
console.log("PASS: api_checkInMember rejects unenrolled member with notEnrolled flag");
...
if (!successRes.success || successRes.data.memberName !== "Member A") {
  throw new Error("api_checkInMember did not succeed for an enrolled member: " + JSON.stringify(successRes));
}
if (appendedAtt.length !== 1) throw new Error("api_checkInMember did not append exactly one attendance row");
console.log("PASS: api_checkInMember succeeds for an enrolled member and returns memberName");
...
if (!dupRes.duplicate || dupRes.data.memberName !== "Member A") {
  throw new Error("api_checkInMember did not detect duplicate check-in: " + JSON.stringify(dupRes));
}
...
const staffEnrollRes = context.api_staffEnrollMember("USER-STAFF", "USER-MEMBER-B", "PROG-1", staffToken);
if (!staffEnrollRes.success) throw new Error("api_staffEnrollMember failed: " + staffEnrollRes.message);
const newEnrollment = enrollments[enrollments.length - 1];
if (newEnrollment[1] !== "USER-MEMBER-B" || newEnrollment[2] !== "PROG-1" || newEnrollment[4] !== "Active") {
  throw new Error("api_staffEnrollMember enrolled the wrong user: " + JSON.stringify(newEnrollment));
}
console.log("PASS: api_staffEnrollMember enrolls the scanned member, not the staff session holder");
...
if (!nowEligibleRes.success) {
  throw new Error("Member B still rejected after staff-enrollment: " + JSON.stringify(nowEligibleRes));
}
console.log("PASS: quick-enrolled member passes the eligibility check on next check-in");

// --- 5. api_searchMembers ---
const searchRes = context.api_searchMembers("member", "USER-STAFF", staffToken);
if (!searchRes.success || searchRes.data.length !== 2) {
  throw new Error("api_searchMembers did not return expected matches: " + JSON.stringify(searchRes));
}
const memberDeniedSearch = context.api_searchMembers("member", "USER-MEMBER-A", memberAToken);
...
if (attNoAuth.success !== false) throw new Error("api_getEventAttendance allowed an unauthenticated read");
const attMemberDenied = context.api_getEventAttendance("EVT-2", "USER-MEMBER-A", memberAToken);
if (attMemberDenied.success !== false) throw new Error("api_getEventAttendance allowed a MEMBER-role read");
const attAllowed = context.api_getEventAttendance("EVT-2", "USER-STAFF", staffToken);
if (!attAllowed.success || attAllowed.data.length !== 2) {
  throw new Error("api_getEventAttendance did not return the granted user's roster: " + JSON.stringify(attAllowed));
}
