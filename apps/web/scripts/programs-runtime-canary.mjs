...
async function assertLocalSecret() {
  const filename = path.join(REPO_ROOT, "apps", "web", ".dev.vars");
  let contents;
  try {
    contents = await readFile(filename, "utf8");
  } catch (cause) {
    throw new CanaryFailure(
      "apps/web/.dev.vars is required for the local Worker canary",
  ...
  if (
    ...
    throw new CanaryFailure(
      "apps/web/.dev.vars must contain a non-placeholder local EFCC_ACCESS_TOKEN_SECRET",
