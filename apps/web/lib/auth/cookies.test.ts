 ...
 *
 * Acceptance covered here (ADR-0020 §2 / apps/web/lib/auth/cookies.ts):
...
describe("AUTH-02: cookie attributes", () => {
  ...
  test("logout clears both cookies", () => {
    ...
    expect(clears.some((c) => c.startsWith(`${ACCESS_COOKIE_NAME}=`))).toBe(
      true
    );
    expect(clears.some((c) => c.startsWith(`${REFRESH_COOKIE_NAME}=`))).toBe(
      true
    );
  });
});
...
describe("AUTH-02: rejection of header/CORS transport", () => {
  test("Authorization header is detected and rejected", () => {
    expect(
      hasAuthorizationHeader(new Headers({ Authorization: "Bearer x" }))
    ).toBe(true);
  ...
  test("cross-origin request is detected and rejected", () => {
    expect(
      isCrossOrigin(new Headers({ Origin: "https://evil.example" }), ORIGIN)
    ).toBe(true);
    expect(
      isCrossOrigin(new Headers({ Origin: "https://efcc.example.com" }), ORIGIN)
    ).toBe(false);
