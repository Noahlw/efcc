...
describe("registrations: role-free contract", () => {
  test("accounts and registration_requests have no role column or write-guard trigger", async () => {
    ...
    const triggers = await db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE 'accounts_role_write_guard%'`
      )
      .all<{ name: string }>();
  ...
  test("approval creates Active role-free account with automatic 會友基礎", async () => {
    ...
    await seedDisposableIdentity(db, {
      databaseName: "E2E_disposable-local",
    }).catch(() => {});
    ...
    expect(
      (account as unknown as Record<string, unknown>).role
    ).toBeUndefined();
  ...
  test("registration queue returns without role", async () => {
    ...
    const rows = await db
      .prepare(
        `SELECT request_id, username, name, phone, account_status, submitted_at, reviewed_by, reviewed_at, review_decision, rejection_note FROM registration_requests LIMIT 1`
      )
      .all();
