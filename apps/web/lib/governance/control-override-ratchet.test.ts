...
function createFixture(): { rootDir: string; baseRef: string } {
  ...
  fs.mkdirSync(path.join(rootDir, "apps/web/app"), { recursive: true });
  fs.writeFileSync(
    path.join(rootDir, "apps/web/app/example.tsx"),
...
describe("T08 incremental control override ratchet", () => {
  test("blocks new owned geometry while allowing layout and safe conditionals", () => {
    ...
    try {
      fs.writeFileSync(
        path.join(fixture.rootDir, "apps/web/app/example.tsx"),
...
`,
  ...
  test("does not audit generated Storybook output as production source", () => {
    expect(
      auditFileContent(
        "apps/web/storybook-static/assets/iframe.js",
