  ...
  for (const contract of contracts) {
    ...
    if (isOwnershipLayer(contract.layer)) {
      ...
      if (
        ...
        contract.scope.startsWith("apps/web/app/")
      ) {
        errors.push({
          code: "INVALID_LAYER_HIERARCHY",
          message: `Primitive contract "${contract.id}" cannot have route scope "${contract.scope}". Primitives belong in apps/web/components/ui/`,
          registry: "contracts",
          entryId: contract.id,
          field: "scope",
          severity: "error",
