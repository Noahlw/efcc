...
function resolveTargetFilePath(f: string, repoRoot: string): string {
  ...
  // Accept both the current apps/web/ layout and legacy web/ callers.
  const stripped = f.startsWith("apps/web/")
    ? f.slice("apps/web/".length)
    : f.startsWith("web/")
      ? f.slice("web/".length)
      : null;
  if (stripped !== null) {
    ...
    const candidate4 = path.resolve(repoRoot, "apps", "web", stripped);
...
function normalizeRepoPath(filePath: string, rootDir?: string): string {
  ...
  // Ensure path starts with apps/web/ if it is inside the workspace.
  const appsIdx = absPath.indexOf("/apps/web/");
  if (!relPath.startsWith("apps/web/") && appsIdx !== -1) {
    relPath = absPath.slice(appsIdx + 1).replace(/\\/g, "/");
  } else if (
    !relPath.startsWith("web/") &&
    !relPath.startsWith("apps/web/") &&
    absPath.includes("/web/")
  ) {
...
export function auditFileContent(
  ...
  if (
    ...
    normalizedPath === "apps/web/.wrangler" ||
    normalizedPath.startsWith("apps/web/.wrangler/") ||
    normalizedPath === "apps/web/out" ||
    normalizedPath.startsWith("apps/web/out/") ||
    normalizedPath.includes("dist/") ||
    normalizedPath === "storybook-static" ||
    normalizedPath.startsWith("storybook-static/") ||
    normalizedPath === "apps/web/storybook-static" ||
    normalizedPath.startsWith("apps/web/storybook-static/") ||
  ...
  const isUiPrimitive = normalizedPath.startsWith("apps/web/components/ui/");
  const isRouteFile = normalizedPath.startsWith("apps/web/app/");
      ...
      while ((match = tagRegex.exec(line)) !== null) {
        ...
        if (!isAuthorized) {
          violations.push({
            ...
            line: i + 1,
            snippet: line.trim(),
            message: `Undocumented native HTML element <${tag}> used in app-facing UI without matching registration in NATIVE_EXCEPTION_REGISTRY. Use shadcn primitives from apps/web/components/ui/ or document native exception in registry.`,
            likelyOwnershipLayer: isRouteFile ? "route" : "pattern",
          });
  ...
  if (isRouteFile && !isTestFile) {
    ...
    if (hasCvaImport || hasCvaCall) {
      ...
      violations.push({
        ...
        line: matchedLine,
        snippet,
        message: `Route-level CVA variant declared or imported in "${normalizedPath}". CVA belongs in primitive (apps/web/components/ui/) or pattern (apps/web/lib/) layers; routes compose existing primitives and patterns.`,
        likelyOwnershipLayer: "route",
      });
    }
  }
...
export function auditSourceCode(options: AuditOptions = {}): AuditResult {
  ...
  if (options.targetFiles !== undefined) {
    ...
    // Scan standard web workspace directory
    const webDir = fs.existsSync(path.join(repoRoot, "apps", "web"))
