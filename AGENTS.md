...
- 100% pass on fresh `/exec` deployment required before `READY`. Assert every criterion via observable DOM state (never assume a click worked). Pipeline appends results to ticket plan (per `ADR-0012`).
