...
- Authenticated E2E = Playwright versus `wrangler dev` on `127.0.0.1:8787` by default (zero Cloudflare account touched). For manual runs, start `pnpm dev:local` and complete `pnpm db:seed:local` plus `pnpm db:seed:demo` before any authenticated request; direct seeding must not race an already-open D1 runtime. The canonical T05 gate is `pnpm verify:programs-runtime`: build, bundle, migrate, direct-seed a disposable D1 with one run-specific `--persist-to` directory before starting exactly one Worker, perform a real disposable-admin login/readiness probe, then run the real-Worker demo seed and unfiltered Programs Playwright journey with retries disabled. Preparation and post-start stages are bounded and SIGINT/SIGTERM cleanup is process-group safe. `PROGRAMS_TARGET_URL` may select another loopback unprivileged port when `8787` is occupied. Unauthenticated/CSS checks use Orca `browser` (`Stateless-Wall` blocks Orca on authenticated RPCs).
- The required full Programs/Worker/D1 prerequisite is `pnpm verify:programs-runtime` after `web/.dev.vars` exists; it owns the local build/migration/reset/seed/listener-readiness sequence and unfiltered single-process Programs journey, preserves each run's logs/results under `test-results/programs-d1-runs/`, and never touches remote resources. The authenticated readiness probe intentionally runs after seeding.
