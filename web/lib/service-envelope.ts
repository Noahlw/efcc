...
export async function verifyServiceEnvelope(
  ...
  let mismatch = 0;
  // eslint-disable-next-line no-plusplus -- loop index is intentional.
  for (let i = 0; i < expected.length; i++) {
    /* eslint-disable no-bitwise -- timing-safe HMAC compare requires bitwise ops. */
    mismatch |=
      (expected.codePointAt(i) ?? 0) ^ (envelope.signature.codePointAt(i) ?? 0);
