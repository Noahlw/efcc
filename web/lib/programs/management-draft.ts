...
/**
 * Session drafts stored for one entity, in storage order. Recovery surfaces
 * use this when the draft's own action identifies its target (for example a
 * Preview Occurrence that is no longer inside the visible range).
 */
export function listManagementDrafts<T>(
 entityId: string
 ...
 const drafts: { action: string; data: T }[] = [];
 try {
  const keys: string[] = [];
  ...
  for (const key of keys) {
   ...
   if (
    ...
    drafts.push({ action: parsed.action, data: parsed.data as T });
   }
  }
 } catch {
  return drafts;
