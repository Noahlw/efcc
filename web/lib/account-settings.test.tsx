/* oxlint-disable vitest/max-expects, vitest/require-top-level-describe, eslint/require-unicode-regexp --
 * Pre-existing account/settings suite: one describe per surface, many
 * observable assertions per journey, and COPY regex locators.
 */
...
vi.mock(import("@/lib/session"), () => ({
  ...
  clearDeepLink: vi.fn<() => void>(),
  rememberDeepLink: vi.fn<() => void>(),
}));
...
describe(ProfileSettingsPage, () => {
  test("renders both forms with labeled inputs, helper texts, and back link", async () => {
    ...
    // Headers & Navigation
    expect(
      screen.getAllByText(ACCOUNT_SETTINGS_COPY.headerTitle).length
    ).toBeGreaterThan(0);
    const backLinks = screen.getAllByRole("link", {
      name: new RegExp(ACCOUNT_SETTINGS_COPY.backToProfile),
    });
    ...
    // Username section
    expect(
      screen.getByRole("heading", { name: ACCOUNT_SETTINGS_COPY.usernameTitle })
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(ACCOUNT_SETTINGS_COPY.usernameLabel)
    ).toBeInTheDocument();
  ...
  test("username change: validates non-empty and succeeds with '登入名稱已更新'", async () => {
    ...
    await expect(
      screen.findByText(ACCOUNT_SETTINGS_COPY.usernameSuccess)
  ...
  test("password change: validates ≥8 chars and mismatch, succeeds with sign-out", async () => {
    ...
    expect(
      alerts.some((el) =>
        el.textContent?.includes(ACCOUNT_SETTINGS_COPY.shortPassword)
      )
    ).toBeTruthy();
  ...
  test("offline attempts for username and password show '未能更新。請重新連線後再試。'", async () => {
    ...
    expect(
      alerts1.some((el) =>
        el.textContent?.includes(ACCOUNT_SETTINGS_COPY.offlineError)
      )
    ).toBeTruthy();
...
describe(ProfilePage, () => {
  test("renders profile info (display name, phone, role, QR code) and settings/logout actions", async () => {
    ...
    const settingsLink = screen.getByRole("link", {
      name: new RegExp(COPY.profile.accountSettings),
    });
