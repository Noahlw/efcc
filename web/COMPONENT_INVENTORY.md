...
- **shadcn used:** `Button`, `Input`, `Textarea`, `Card`, `ActionSurface`, `RouteHeader` via Tailwind/token utilities; `home-cms-editor.module.css` deleted in Phase E and replaced with Tailwind — no module CSS remains. Template chooser and publish actions use `Button` with `aria-pressed` as before.
