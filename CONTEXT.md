...
| Member Baseline | 會友基礎 | The system-owned participant policy applied to every Active Account regardless of whether its global Role is `Admin`, `Staff`, or `Member`. It is not a Role, cannot be assigned, reordered, or edited, and expresses that management authority adds to rather than replaces ordinary church-member participation. |
| Approval Selection | 審批選取集 | The explicit set of Pending Registration Request IDs an operator has selected for one batch decision. It may accumulate across scroll, search, filters, and a Detail round trip within the Approvals module; hidden entries remain reviewable, and reload, logout, module exit, or explicit Clear ends the selection. |
| Registration Batch Approval | 批次註冊核准 | One all-or-nothing decision over an Approval Selection. Every request must still be Pending at commit time; any stale or conflicting entry leaves every selected request unchanged. Successful decisions create Active Accounts and immutable per-request audit outcomes in the same transaction. Bulk rejection is not part of this term. |
...
| 0040 | Discord-Derived S4 Management Interaction Authority | Accepted |
| 0041 | Atomic Registration Batch Approval | Accepted |
