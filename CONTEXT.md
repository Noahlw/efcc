...
| Church Member | 會友 | A person participating in the church's member experience. Every Active Account, including one whose global Role is `Staff` or `Admin`, retains the Church Member participant baseline; management authority is additive rather than a replacement for member access. |
| Role | 角色權限 | The global management-authority tier stored in the `System_Role` column of the `Users` sheet (legacy) or the D1 `accounts` global role. The canonical values are **`Admin`**, **`Staff`**, and **`Member`** (default when empty). One account has exactly one global Role, but every Role retains the Church Member participant baseline; `Staff` and `Admin` add management authority rather than replacing member access. The D1 schema and auth/API contract currently contain the older `Teacher` spelling and require a migration before deployment; `STAFF` is the normalized code representation of the canonical `Staff` value. **Program Leader** and **Department Manager** are scoped effective-access profiles, not global Role values. |
...
| Account Directory | 帳戶名錄 | The identity-and-access view of church Accounts. It may expose login identity, contact details, global Role, account status, and Department associations, but it is not a pastoral member record, a Permission Policy, or an account-lifecycle mutation surface. |
| Church Member Directory | 會友名錄 | The church-person record view used for pastoral and member-oriented information. It is distinct from the Account Directory even when both refer to the same person, because church membership and login access have different ownership and lifecycle. |
| Registration Approval | 帳戶註冊審批 | The decision that accepts or rejects a pending self-service Account registration. Approval activates an Account; it is distinct from Enrollment Approval, which governs participation in a Program. |
...
| Permission Policy | 權限政策 | The configurable mapping from each global Role (`Admin`, `Staff`, or `Member`) to additive management Capabilities on top of the shared Church Member participant baseline. It contains no per-account overrides; Department Manager and Program Leader grants remain separate scoped authority that the server combines when projecting effective access. Browser affordances may reflect that projection, but server authorization remains authoritative. |
| Scanner Section | 掃描功能區 | The D1 self-check-in surface at `/scanner`, labelled **掃描** in the dock and page header. On phone-width entry it opens the camera immediately; the live mode is a dark full-screen surface with persistent Shared Shell navigation, while manual Event Check-In Code and Member QR assistance appear only after stopping or failing camera startup. At desktop width the Section exposes manual code entry only, not the camera Scanner page. The Worker owns enrollment and check-in-window authorization; assisted check-in remains server-authorized. |
| Camera-first Scanner Entry | 相機優先簽到入口 | The plain phone-width `/scanner` state that starts camera capture and QR decoding immediately on entry. It contains one short instruction, one middle scan frame and one `停止掃描` action; it does not show fallback method cards until the user stops scanning or camera startup fails. Credential deep links skip camera and resolve their known intent directly. |
| Camera Permission Denied | 相機權限未開啟 | A recoverable camera-start state caused by the browser refusing or withholding camera permission. It explains that permission is not enabled, offers `重試相機`, and keeps equal fallback controls for `輸入代碼` and `出示會員 QR`; unsupported devices do not use this term or promise retry. |
| QR Decode Capability | QR 解碼能力 | The guarantee that camera QR scanning is available on every supported browser. Where the platform provides a native Barcode Detection API the app uses it directly; where it does not — WebKit and Firefox, and therefore every iOS device — the app lazily loads a same-origin WebAssembly decoder instead. The decoder is never fetched from a third-party origin, and it is never downloaded by a browser with native support. Typing an Event Manual Check-In Code remains available as a fallback, while Member QR assistance is the operator-mediated alternative. |
