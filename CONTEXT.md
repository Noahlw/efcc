...
| Credential Reset | 憑證重置 | An Admin/Staff-authorized replacement of an account's credential for a member who cannot log in. The system generates a temporary credential shown to the operator exactly once; the member's first login forces them to set their own password before any Session is issued. Reset clears the legacy-PIN upgrade gate and lockout state, revokes all refresh sessions, requires a reason recorded in audit history, and never changes account_status. Only salted PBKDF2 hashes are stored — never plaintext credentials. |
