...
| `spreadsheet-access.gs` | `efccSpreadsheet_()` — opens the app's Spreadsheet by ID (`EFCC_SPREADSHEET_ID` Script Property) rather than `SpreadsheetApp.getActiveSpreadsheet()`, because the script is standalone, not bound (ADR-0015). Used by all three repositories below. |
...
| `appsscript.json` | Manifest: V8 runtime, `webapp.access = ANYONE_ANONYMOUS`, `webapp.executeAs = USER_DEPLOYING`. |
