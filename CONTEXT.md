...
| Schedule Rule | 時間表規則 | An editable recurrence pattern owned by a Recurring Program that produces its Events. Generating Events from rules is an audited run with created/skipped counts; generating when no rule exists is invalid. |
...
| Management Workspace | 管理工作 / 管理工作台 | The dedicated operational surface for permitted accounts, reached via the management dock tab or program-level mode switch. It comprises the grouped Management Hub Directory (會員與權限、事工營運、內容與系統), per-program Course Cockpits, department configuration, and content/settings modules. |
| Course Cockpit | 課程工作台 | The per-program operational command center (`ProgramWorkspace`) providing a status-first overview: next meeting card with live check-in progress (`已簽到 x/y`), 2-up operational tiles (`聚會` and `參與者` with pending badges), and quiet low-frequency settings and facts links. |
| Task | 任務 | A URL-addressable sub-view inside a Course Cockpit or Management Hub area, one level below a Section (`ProgramsTask` in code). A given design-reference screen becomes its own Task only when it runs an independent multi-step flow the user can get lost in (search-then-commit, a multi-field editor); a single-record yes/no decision stays inline regardless of use frequency. See ADR-0033. |
| Attention Center | 待處理與通知中心 | The universal top-right bell overlay accessible to all authenticated accounts. It cleanly separates server-projected actionable operational work (`待處理`, numbered badge) from informational announcements (`通知`, unread dot) without conflating task resolution with message read-state. |
| Home Content CMS | 首頁內容管理 | The in-app administrative publishing module under Management Hub supporting two fixed phone-safe layouts (Template A: featured upcoming event with next-eligible fallback; Template B: church announcement with title, summary, sanitized rich body, CTA, and editorial image), independent drafts, immediate or scheduled HK-time publishing, optimistic-concurrency conflict protection, and immutable audit trails. |
| 5-Slot Bottom Navigation | 五格底部導航 | The unified phone navigation dock providing five stable destinations based on account capability: Participant dock (`首頁 · 課程 · 〔掃描〕 · 通知 · 帳戶`) or Management-role dock (`首頁 · 課程 · 〔掃描〕 · 管理 · 帳戶`), with the central raised Scan button. |
| Account Subscriptions | 消息訂閱偏好 | The persisted D1 record (`account_subscriptions`) tracking an account's opt-in preferences for church-wide editorial announcement topics and ministry communications, distinct from operational transactional notifications. |
| Scheduled Content Cron | 排程發佈定時器 | The 5-minute Cloudflare Scheduled Worker cron trigger (`*/5 * * * *`) that evaluates active/expired scheduled Home Content versions against `Asia/Hong_Kong` church time, transitioning published states and falling back to previous valid versions upon expiry. |
| Member Directory Scope | 會員名冊權限範圍 | The capability-gated visibility boundary for the Management Hub Member Directory: Admin and Staff have full church-wide search across Active accounts, while Department Managers see only members enrolled in programs under their assigned departments. |
| Template B Format | 版面 B 格式規範 | Structured church announcement content authored in sanitized Markdown (supporting H2/H3 headings, bold, italic, lists, blockquotes, links, and validated external HTTPS image URLs) strictly filtered against raw HTML, scripts, and embeds. |
| Event Cancellation Guardrail | 聚會取消防護 | The invariant requiring all active Attendance records for an Event to be voided with reasons before that Event can be cancelled. Once cancelled, an Event cannot accept new check-ins and retains its full history. |
| Guest Attendance Correction | 訪客出席更正 | An in-place update to a guest's name or phone on an active Attendance row by an authorized leader, recording a `GUEST_ATTENDANCE_CORRECTED` immutable audit event with old/new values, actor, timestamp, and explanation. |
| Department Archival Guardrail | 部門封存防護 | The invariant blocking Department archival while any child Program remains in `Draft` or `Active` status. All child Programs must be archived first. |
| Program Archival Invariant | 課程封存規則 | Program archival closes operational and enrollment commitments; it preserves all historical records read-only and is reversible exclusively by an Admin with a recorded audit reason. |
| Print Sheet Privacy | 簽到單隱私保護 | Physical printed attendance sheets (M-22) mask middle phone digits (e.g. `9123****`) to safeguard member personal data at physical venues while preserving full numbers in-app for authorized leaders. |
| Scanner Fallback Contract | 掃描器降級契約 | Automatic inline transition to the 6-character Event Manual Code input screen (P-09) when camera access is denied, unsupported, or unavailable, with a persistent link to manual code always accessible. |
...
| 0032 | Management Workspace Design Authority: Full Prototype Adoption & 5-Slot Shell | Accepted — prototype and two-prototype consolidated plan govern Management Workspace screens, 5-slot bottom dock, Attention Center, Course Cockpit, and Home CMS |
| 0033 | Course Cockpit and Management Hub Task Granularity | Accepted — refines ADR-0032's 55-screen count from a route mandate to a design-reference count, governed by a frequency/independent-state rule |
