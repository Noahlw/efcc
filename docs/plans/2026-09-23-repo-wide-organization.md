...
The Programs Browser gate discovers **70 items**: 35 management tests in `phone-390`, four participant tests in each of `phone-360`, `phone-390`, and `phone-402`, and 23 navigation tests in `phone-390`. The management flows run once at the representative phone width because the separate responsive matrix owns viewport geometry. The migration manifest maps the formerly uncovered cases #9, #14, #25, #26, #29, #30, #35, and #38 to current responsive, participant, or management journeys, and validates their report titles and project placement. An earlier focused participant run stalled during local D1 setup; after the application and assertion fixes below, the complete dirty-worktree Browser run passed 70/70 with zero retries. Clean-candidate promotion is still pending.
