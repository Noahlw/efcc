...
On `c28b70616336afc0d8d5b246f8b0483526759c42`, a later local promotion rerun failed Browser case #47 at 69/70 (`test-results/programs-promotion/20260925t080824658z/promotion.json`). Its trace showed a 200 Events response containing the target event arrived as the row assertion timed out while the page still showed loading. The test now registers a wait for that Events read before navigation. The dirty-worktree diagnostic `test-results/programs-browser-acceptance/20260925t100215871z/run.json` passed 70/70 with zero skips, flaky results, or retries; it does not replace a clean-candidate aggregate.
