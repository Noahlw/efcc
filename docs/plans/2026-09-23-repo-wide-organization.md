...
The four previously failing management cases were then rerun together and passed 4/4. After that, `pnpm test:programs:browser` passed **70/70**, with zero skips, unexpected results, flaky results, retries, or report errors (Playwright duration 216.8 seconds). The JSON report is `test-results/programs-browser-acceptance/20260924t145352110z/browser-results.json`. This was a dirty-worktree diagnostic run: its manifest reports HEAD `8beec645e316fd95e96d2f7f599a23919abb17ac`, so it is not clean-candidate acceptance evidence. Run `pnpm verify:programs` after committing the complete candidate; that gate enforces a clean worktree and reruns the parity stages.
