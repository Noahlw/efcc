...
**Status:** The original repo-wide candidate `fc0cf979b2e16630eac34fb15580c2c184a491ec` passed the full local `pnpm verify`; frozen install and build also passed at that candidate. The independent branch review's stale scanner/auth findings were marked historical. A further cross-reference review marked the completed PR #166 acceptance trace historical, retired the PIN Upgrade row in the accepted #179 design contract, and marked ADR-0014/0044 CI provisions as superseded by ADR-0056. Programs parity and Actions/ruleset retirement are complete. The 2026-09-25 second pass adds live-session validation and immediate revocation; password-login rate limiting, Cloudflare target identity, B-003 canary, owner/device review, and the four-task agent-context comparison remain open. No token-saving or performance claim is made.<br>
...
This plan consolidates the owner decisions reached during the 2026-09-23 grilling and resolves the actionable CEO-review findings. It governs the approved implementation on `refactor/repo-wide-organization-implementation`. The user authorized local source, test, configuration, and documentation changes, including rebuilding this worktree's disposable local D1. In the current continuation, the user also authorized pushing this branch, opening a PR, reviewing it, and fixing review findings. Merge, deployment, release, and unknown remote D1 changes remain unauthorized.
...
### Second-pass Auth request validation

The user approved this focused Auth cleanup on 2026-09-25 while keeping provider/library migration and password-login rate limiting in #639. Preserve the current HMAC access-token format and `EFCC_ACCESS_TOKEN_SECRET`. The variable is EFCC's application signing key; Cloudflare does not prescribe its name, but the Worker needs it to sign and verify tokens. Keep the local `.dev.vars` setup for this required secret.

Protected requests verify the HMAC first, then make one prepared D1 query joining the session and account by `sid`/`uid`. Deny missing, revoked, or idle-expired sessions and missing accounts. This makes single-session and all-session emergency revocation effective on the next protected request while rejecting bad signatures before a D1 read. Preserve caller-specific response statuses and wording, Attendance's optional-auth behavior, Home CMS's publish-capability check, 15-minute access-token and 90-day refresh lifetimes, refresh rotation, cookies, role/grant outcomes, and audit behavior. A retry using a session revoked by its first credential-change request now follows the existing invalid-session 401 path; a same-value username no-op with a still-valid session remains 200. ADR-0058 supersedes only ADR-0020 §2's stateless-revocation claims.

The Worker/D1 acceptance must prove a live session, tampered/expired access-token rejection, single-session revocation with another device still valid, and all-session revocation denying the next request. This adds a D1 read after each valid access-token signature; do not claim a performance improvement. Keep the direct D1 primary-read path. Cloudflare documents read-replica consistency through its [D1 Sessions API](https://developers.cloudflare.com/d1/best-practices/read-replication/); revisit revocation consistency before adopting read replication.

Password-login brute-force protection and Auth-provider/library migration remain pre-production work in [#639](https://github.com/Noahlw/efcc/issues/639). The provider decision map and its child do not themselves implement either safeguard.

## Original repo-wide implementation candidate

The implementation branch is `refactor/repo-wide-organization-implementation`, based directly on `origin/main` at `b18828d42b6e152f2d29ed1f63df9c1681e59046`; `origin/main` was rechecked on 2026-09-25 and is unchanged. Before the Auth/session second pass, the last code/workflow candidate was `fc0cf979b2e16630eac34fb15580c2c184a491ec`. On that exact SHA, `pnpm verify` exited 0, the frozen workspace install reported `Already up to date`, and the Next production build passed as part of the aggregate. The promotion manifest is `test-results/programs-promotion/20260925t023829145z/promotion.json`. No merge, deployment, or remote D1 mutation has occurred. Ruleset `20586715` was updated and read back as recorded below.

Completed local slices include the single pnpm workspace and lockfile, local-only D1 baseline/reset path, retired prototype/scanner and Google Users/PIN runtime, Home-origin Browser source, removal of the old credential-upgrade Storybook fixture, module ownership cleanup, current documentation/ADR updates, and the bounded Drizzle trial (not adopted). The current production build excludes `/prototype` while preserving `/scanner`; root frozen install and the full local gate passed on candidate `fc0cf979b2e16630eac34fb15580c2c184a491ec`. This qualifies local implementation checks only; it does not qualify external deployment identity, owner/device review, or release.

The Programs Browser gate discovers **70 items**: 35 management tests in `phone-390`, four participant tests in each of `phone-360`, `phone-390`, and `phone-402`, and 23 navigation tests in `phone-390`. The management flows run once at the representative phone width because the separate responsive matrix owns viewport geometry. The migration manifest maps the formerly uncovered cases #9, #14, #25, #26, #29, #30, #35, and #38 to current responsive, participant, or management journeys, and validates their report titles and project placement. The earlier dirty-worktree 70/70 Browser run was diagnostic only; clean-candidate promotion and the full local aggregate are recorded in the Agent handoff below.
...
Implemented: `tests/e2e/programs-home-acceptance.test.ts` and its config provide five PUI-05 Home-origin browser cases, plus seven feed cases in a separate runner stage. Both use `wrangler dev --local` with a per-run disposable D1 persistence directory and deterministic seeded content. The promotion manifest requires exact old-ID to replacement-test mappings, the expected stage counts, zero skips/retries, and local Worker/D1 identity. Implementation amendment (2026-09-24): the Browser stage expects 70 items: 35 management at phone-390, 12 participant items across three phone projects, and 23 navigation at phone-390. The separate 21-case responsive and five-case Home stages own viewport geometry and Home-origin journeys. Earlier 48-item runs below are historical evidence from before the expanded management and navigation parity slice.
...
At the time of this 2026-09-24 diagnosis, the repository's official `pnpm test:programs:browser` runner was rerun on the same SHA with a fresh disposable local Worker/D1 harness: **48/48 passed**, zero skips/retries/flakes; #26 passed at 360, 390, and 402px. No product or test assertion was changed. The then-current failure was not reproduced; this historical 48-case diagnostic was superseded by the expanded clean-candidate promotion recorded in the Agent handoff below.
...
### Programs Browser parity exploratory run (2026-09-24)

The first run of the expanded management replacement suite used the earlier three-viewport configuration and was interrupted after roughly 12 minutes, once the same deterministic assertion failures were repeating across phone projects. It exited with code 130 before writing a Browser JSON report. Its `run.json` still says `running` because interruption bypassed the runner's finalizer; the artifact is invalid and is not a pass. Trace inspection found a stale #39 assertion that expected one department-settings button per department even though the UI has one launcher, mismatched #42/#62 response-code expectations, #43 archive expecting a different current error status, and missing/untimely controls in several new management journeys. Some `ERR_ABORTED` and Miniflare `Network connection lost` messages also occurred during local teardown; keep these separate from deterministic assertions.

The duplicate management viewport runs have since been removed: management journeys run at `phone-390`, while the dedicated responsive matrix owns geometry at multiple widths. Discovery reports 70 items (35 management, 12 participant across three phone projects, and 23 navigation). A focused participant attempt stopped during local `createTestHarness` D1 migration/seeding before `fixture-seed.json`; its report is invalid. The independent management review identified parity gaps in #39, #40, #47, #30/#53, #55, #59/#60, and #63. Those and the rejected-account cleanup issue have now been repaired. The focused #14/#25/#26 participant subset passed 9/9 using the corrected local Worker/D1 runner described below.

The four previously failing management cases were rerun together and passed 4/4. `pnpm test:programs:browser` then passed **70/70** with zero skips, unexpected results, flaky results, retries, or report errors (216.8 seconds); its report is `test-results/programs-browser-acceptance/20260924t145352110z/browser-results.json`. That was a dirty-worktree diagnostic on HEAD `8beec645e316fd95e96d2f7f599a23919abb17ac`, not candidate-bound acceptance. On clean candidate `5d8c660063f1a811df647844c6051c5594a594b0`, Worker Contract and Browser Acceptance passed, then Home case 66 exposed a strict-mode locator that selected four feed elements instead of the intended list. The runner reached the local Worker/D1 and all setup/migration requests succeeded; no Wrangler bootstrap or D1 initialization error occurred. These findings were corrected and superseded by the clean final promotion and repository-wide verification recorded in the handoff below.

The approval queue detail link uses Next.js `scroll={false}` and restores the nested `#shell-content` scroller on browser Back, then restores focus without scrolling. Context7 resolution/query was unavailable at the monthly quota limit on 2026-09-24 UTC; Firecrawl read the [current official Next.js Link documentation](https://nextjs.org/docs/app/api-reference/components/link), updated August 25, 2026, which documents `scroll={false}` as disabling Next's automatic scroll-to-page behavior. Cloudflare's official [Workers local development](https://developers.cloudflare.com/workers/local-development/) and [D1 local development](https://developers.cloudflare.com/d1/best-practices/local-development/) docs (queried 2026-09-24 UTC; Workers page updated August 20, 2026) confirm `wrangler dev --local` uses local bindings and disables remote bindings. The runner also passes `--local` and per-run `--persist-to`; no Wrangler or remote-resource issue was observed.

After the Home case 66 locator correction, `pnpm test:programs:home` passed on the dirty worktree (report `test-results/programs-home-acceptance/20260924t160443584z/home-results.json`). The next dirty Browser Acceptance run passed 67/70; cases #52 and #62 were assertion-locator mistakes, and #63 did not reach its detail route in that full run. The three cases passed 3/3 in a focused local Worker/D1 rerun after locator fixes, including a real browser Back scroll check for #63. These runs are diagnostic only because they include uncommitted changes; rerun the full promotion gate on a committed clean candidate before claiming acceptance.
...
- **GitHub workflow retirement completed (2026-09-25):** repository owner access was verified as `ADMIN`; ruleset `20586715` was read before and after the update. Only its `Fast CI` required-status rule was removed; the active ruleset still enforces deletion and non-fast-forward protection. The branch deletes `fast-ci.yml`, `ui-governance.yml`, `e2e.yml`, and the now-unneeded `.github/CI-SECRETS.md`. The old workflows referenced the retired `web/pnpm-lock.yaml` or nested `pnpm --dir web install` path. The ruleset change is already live; the workflow deletions will reach `main` when this PR merges.
...
- Password-login brute-force/rate-limit policy remains pre-production work for #639. Emergency revocation is addressed by this second pass; the provider decision map does not implement either item.
- The five-minute `pnpm test:programs:canary` was not run. B-003 remains a separate runtime diagnostic and is not part of `pnpm verify`.
- The plan's four matched coding-agent tasks (Programs, UI, deployment/config, query/field) were not measured before/after. Coding-agent context/token reduction is therefore unquantified; do not claim token savings.
- The plan's owner, real-device, Cloudflare target, deployment, and release reviews remain separate gates.
- Historical note (2026-09-23/24): three missing triage labels were created and read back through the authenticated GitHub web UI. At that time, the selected CLI collaborator had no repository-admin permission; the owner account was verified as `ADMIN` for the 2026-09-25 ruleset update recorded above.
...
Cloudflare plugin documentation search (2026-09-24; Context7 Cloudflare Workers library resolution returned “Monthly quota reached,” so no Context7 library ID was available):
- [Integration test harness guide](https://developers.cloudflare.com/workers/testing/test-harness/) and [Wrangler API](https://developers.cloudflare.com/workers/wrangler/api/) describe `createTestHarness()`; Cloudflare recommends it for integration tests instead of `unstable_dev`/`unstable_startWorker`.
- [Configure the test harness](https://developers.cloudflare.com/workers/testing/test-harness/configure/) and [Get started](https://developers.cloudflare.com/workers/testing/test-harness/get-started/) show configuring a local Worker from Wrangler and exercising it through the test harness.
- [Migrate from `unstable_dev`](https://developers.cloudflare.com/workers/testing/vitest-integration/migration-guides/migrate-from-unstable-dev/) repeats that recommendation.
- [Workers Vitest configuration](https://developers.cloudflare.com/workers/testing/vitest-integration/configuration/) and [D1 test APIs](https://developers.cloudflare.com/workers/testing/vitest-integration/test-apis/) document loading Wrangler D1 migrations and applying them to a test binding.
- [D1 local development](https://developers.cloudflare.com/d1/best-practices/local-development/) describes testing Worker+D1 locally with Wrangler before deployment.

The original Browser runner used `createTestHarness()` and seeded its local `DB` binding from Node. That exploratory run was interrupted and produced no valid report. A follow-up reproduced the setup stall at host-side `applyD1Migrations()` / `getEnv()` calls with the configured D1 binding, before Browser tests began; an earlier teardown `Network connection lost` remains a separate, unclassified observation. The current implementation amendment below addresses the code path without changing product Worker behavior or the real Worker/D1 acceptance boundary.

### Local Worker/D1 runner correction (2026-09-24)

The installed Wrangler 4.131.2 `createTestHarness()` path blocks in Node-host D1 migration/binding access for this runner, while the Worker HTTP request path remains reachable. Its current implementation sets `dev.persist: false`, so `wrangler d1 --local` CLI state cannot seed that in-memory harness. Browser, Home/Feed, Responsive, and Runtime Reliability runners now use the configured Worker directly through `wrangler dev --local`; Wrangler CLI applies migrations and generated E2E SQL using the same per-run temporary directory passed to `--persist-to`. Shutdown stops the Worker and removes the temporary database. This keeps browser and canary checks on actual local Worker/D1 code with no remote target or product-only test route.

Evidence on the dirty implementation candidate: setup smoke applied 38 fixture-reset statements and 9 seed statements to an isolated local D1, reached the configured Worker over HTTP, and cleaned up; participant parity #14/#25/#26 passed 9/9 across phone-360/390/402. This does not qualify the full 70-item Browser stage or `pnpm verify`.
...
- [Cloudflare Workers integration test harness](https://developers.cloudflare.com/workers/testing/test-harness/)
- [Wrangler `createTestHarness()` API](https://developers.cloudflare.com/workers/wrangler/api/)
- [Cloudflare Workers `createTestHarness` migration guidance](https://developers.cloudflare.com/workers/testing/vitest-integration/migration-guides/migrate-from-unstable-dev/)
...
- [Cloudflare test harness: prepare test state](https://developers.cloudflare.com/workers/testing/test-harness/prepare-test-state/) — documents `applyD1Migrations()` followed by `getEnv()` for harness D1 seeding; the current configured host-side D1 path stalled, so the E2E runner now uses Wrangler's local development runtime.
- [Cloudflare D1 local development](https://developers.cloudflare.com/d1/best-practices/local-development/) — documents `wrangler dev --persist-to` and local D1 use. Wrangler 4.131.2 CLI help confirms `--persist-to` is supported for local `migrations apply` and `execute`.
- [Playwright Test configuration](https://playwright.dev/docs/test-configuration) — project-specific `testIgnore` supports multiple glob patterns.
...
For the 2026-09-25 recheck, the Cloudflare Docs connector confirmed that [matching Static Assets](https://developers.cloudflare.com/workers/static-assets/) are served before Worker code by default, [local Worker bindings](https://developers.cloudflare.com/workers/local-development/) use local simulations, and `wrangler dev --local` disables remote bindings. [D1 local development](https://developers.cloudflare.com/d1/best-practices/local-development/) describes isolated local data and `--persist-to`. The browser runners use `--local` with a per-run temporary persistence directory; no deployment configuration or remote target was changed. Context7 again returned its monthly quota limit, so no Context7 library ID was available.

## Agent handoff

The earlier candidate-bound acceptance record is in [evidence.json](../../.delivery/repo-wide-organization-implementation/evidence.json); it records code candidate `0a5112c3f91f67a724e001fa6c37469089ac3ee0` and its remaining gates. The original pre-Auth second-pass full local aggregate is the `fc0cf979` promotion manifest linked above. Neither record is current PR-head evidence or makes a token-savings claim; see [PR #644](https://github.com/Noahlw/efcc/pull/644) for later candidate-bound results.

The approved local implementation is active on `refactor/repo-wide-organization-implementation`. The single workspace, local D1 baseline, retired prototype/import paths, Home/Feed Browser slices, docs, selected test reductions, and Drizzle stop decision are implemented; raw SQL and Wrangler migrations remain. The old Programs and PUI-05 Home-origin suites/config and their unused event-window helper were retired after clean-candidate parity passed.

The original complete local `pnpm verify` exited 0 on candidate `fc0cf979b2e16630eac34fb15580c2c184a491ec` using Node 22.18.0 and pnpm 11.7.0. It passed 659 Worker tests, 1,184 component tests, governance/source audit (426 files, zero active violations), Storybook scope/index, and the T05.7 stages: Worker Contract, 70 Browser items, five Home items, 21 Responsive items, seven Feed items, and non-browser precommit. The browser runner and promotion manifest require zero retries. Shell-responsive passed 92 with one configured skip; shell geometry passed 35; role-hierarchy geometry passed 49. Storybook index reconciliation passed with 95 Stories and 36 Screen Catalog obligations, and the retired credential-upgrade Story ID is absent. `pnpm install --frozen-lockfile` and the aggregate's production builds passed on this candidate; static export includes `/scanner` and excludes `/prototype`. After that candidate, documentation-only changes clarify Specs 072, 078, 079, and 084 and supersede old CI policy text in ADR-0014/0044; the full governance audit and whitespace check were rerun. The five-minute canary was not run and remains B-003.

The #30 browser test now waits for the participant snapshot response and the section's settled state before switching to Pending, then scrolls the detail button into view before clicking. The #63 browser test also scrolls the shell first and waits for its detail row to settle; the Home Feed fixture marks existing notices read before creating three unique notices, then asserts each row's timestamp/unread state and mark-all-read persistence. No runtime API/schema changed in these test repairs. An independent branch review at `e7dd15506c9e391949a8bfd41c3a17887743c644` found no confirmed production code, permission, or D1 schema regression, but identified stale current-looking Specs 072, 075, and 084; these now distinguish the retired paths. A later source cross-reference found the completed #166 trace and PIN Upgrade row in Spec 079 also needed historical status; both are now marked, and ADR-0014/0044 point to ADR-0056 as the current CI policy. Those reviews did not establish independent full Worker/D1/browser acceptance. Emergency revocation is included in the second pass. Keep Auth-provider work deferred and do not claim production readiness while password-login rate limiting, deployment identity, or owner/device gates remain open.

On `c28b70616336afc0d8d5b246f8b0483526759c42`, a later local promotion rerun failed Browser case #47 at 69/70 (run `20260925t080824658z`). Its trace showed a 200 Events response containing the target event arrived as the row assertion timed out while the page still showed loading. The test now registers a wait for that Events read before navigation. A dirty-worktree Browser diagnostic (run `20260925t100215871z`) passed 70/70 with zero skips, flaky results, or retries; it does not replace a clean-candidate aggregate. A subsequent shell Playwright run erased both ignored run artifacts because its default output directory was the shared `test-results/` root. The three shell/geometry configs now use scoped output directories so later stages preserve the promotion manifest.

The ruleset update is verified. This handoff does not authorize merge, deployment, or remote D1 mutation. The connected Cloudflare account has no visible D1/Worker resources; the configured Worker, route, D1, and rate-limit identities remain unverified, so do not change them or deploy.
