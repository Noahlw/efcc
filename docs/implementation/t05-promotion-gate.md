...
The owner-approved removal of all three GitHub Actions workflows remains a separate governance item because the current `main` ruleset also requires the `Fast CI` check. Remove the required check and the workflows in one authorized window; until then, the existing workflow files remain. `pnpm verify` is the intended local readiness gate and does not establish deployment qualification.
