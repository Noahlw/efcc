...
| Phase status | `IN PROGRESS — T01–T04 safe prefix merged into rescue; T05 CANONICAL RUNTIME MIGRATION in progress; T06 remains T05-gated` |
...
| Implementation frontier | T05 / #510 `IN_PROGRESS — CANONICAL RUNTIME MIGRATION`; the prior shell/Harness qualification evidence is preserved for Task 4 classification; T06 remains provisional and final validation/merge remains gated by T05 |
| Merge frontier | T05 / #510 Canonical migration → T06 / #511; #549 remains Draft until the Canonical contract is proven, qualified, reviewed, and green |
...
| Active blocker | T05 Canonical migration and qualification are pending. B-003 ProxyWorker evidence remains preserved as historical Diagnostic/upstream evidence; T06 remains T05-gated for final validation, `STACK_GREEN`, and merge |
| Next safe action | Execute T05 Task 2: make the Harness runner testable; preserve the existing shell runner as Diagnostic and keep T06 final validation/merge gated by T05 |
...
| T05 | [#510](https://github.com/Noahlw/efcc/issues/510) | Stabilize full Programs/Worker/D1 runtime | T04 | `IN_PROGRESS — CANONICAL RUNTIME MIGRATION` | [#549](https://github.com/Noahlw/efcc/pull/549) Draft | — | Stable implementation `983dbf95`; later `493d41ed` and `18623fa3` commits are documentation-only follow-ups. The official `createTestHarness()` path is now the authorized Canonical target; the existing shell runner remains Diagnostic, prior Red evidence is preserved for Task 4 classification, and no acceptance threshold or production behavior changes |
...
| Open blockers | T05 Canonical migration and qualification are in progress; B-003 remains preserved as historical Diagnostic/upstream evidence. T06 may remain provisional under D-004, but cannot merge or become `STACK_GREEN` until T05 is green |
...
| Implementation frontier | T05 / #510 is `IN_PROGRESS — CANONICAL RUNTIME MIGRATION`; prior repo-owned lifecycle and bounded Harness evidence remain available for the migration; T06 remains provisional under D-004 |
| Merge frontier | T05 / #510 Canonical qualification → T06 / #511; #549 remains Draft pending the complete Canonical runtime proof |
| Review pending | None for T05 implementation; Standards and Spec two-axis review passed. Canonical migration, root-cause classification, and five-run qualification remain pending; B-003 evidence is preserved |
...
| Next safe action | Execute T05 Task 2: make the Harness runner testable; final T06 validation and merge remain gated by T05 |
...
| 3 | T05 / #510 | T04 | `rescue/ui-control-recovery` | `rescue/t05-runtime-stability` | `/Users/noah.wong/Desktop/code/EFCC-dev/.worktrees/phase0-t05-20260904` | `rescue/ui-control-recovery` | [#549](https://github.com/Noahlw/efcc/pull/549) Draft | `IN_PROGRESS — CANONICAL RUNTIME MIGRATION` | `983dbf95` | N/A | `b757fdc4e4714514df9baf552e47eaddf10d289d` |
...
- **Status:** `IN_PROGRESS — CANONICAL RUNTIME MIGRATION`
- **Base rescue SHA:** `b757fdc4e4714514df9baf552e47eaddf10d289d` (current rescue HEAD; T05 restacked)
- **Current implementation SHA / merge SHA:** `983dbf957d67969db6b591bb9af61ce5c9032298` / pending
...
- **Delivered outcome:** The existing Diagnostic shell runner builds and bundles the Worker, creates a unique per-run `wrangler-state` persistence boundary, applies migrations, completes direct disposable identity seeding before the long-lived Worker starts, waits for listener and authenticated readiness, seeds demo state through the real Worker API, runs the complete unfiltered Programs journey, and preserves logs/results. Demo and Programs targets are explicitly identical; retries are disabled for this gate. Preparation/post-start stages have bounded timeouts and signal-safe process-group cleanup. The official `createTestHarness()` implementation is now the authorized Canonical migration target. Production Programs code matches the corrected T03 baseline exactly — speculative request coalescing and audit batching remain reverted, with no independent EFCC defect evidence and no production behavior change.
- **File classification:** RUNNER / TEST INFRA — `package.json`, `scripts/run-programs-acceptance.ts`, `scripts/run-programs-acceptance.test.ts`, `tests/e2e/programs-d1.config.ts`, `tests/e2e/seed-local-d1.ts`, `tests/e2e/seed-disposable-identity.ts`, `web/package.json`, `web/wrangler.jsonc`, `web/scripts/qualify-test-harness.mjs`; CI — `.github/workflows/e2e.yml`; DOCUMENTATION — `.github/CI-SECRETS.md`, `AGENTS.md`, `CONTRIBUTING.md`, `tests/e2e/README.md`, this tracker; INDEPENDENT CORRECTNESS REGRESSION — `web/lib/programs/programs.test.ts`; PRODUCTION BEHAVIOR CHANGE — none.
- **Tests:** T05 runner tests 19/19; root and web typechecks; the aggregate commit gate passed prototype 38, governance CLI 18, identity 98, Worker 605, components 889, and full governance with 279 files scanned, 0 active violations, and 75 waivers. Shell attempt `20260903t223541671z` completed build, bundle, migration, direct seed, listener/auth readiness, and demo seed, then recorded Worker exit code 1 with first cause `Error in ProxyController: Error inside ProxyWorker` / `Network connection lost.` and 168 downstream `ERR_CONNECTION_REFUSED` signals; its Playwright process recorded 23 passed before the Worker died. Earlier deterministic attempts `20260903t203931257z` and `20260903t204826297z` recorded the same first-cause signature. A bounded shell run `20260903t222937007z` reached 201/201 Playwright pass but exposed the intentional-SIGINT runner false negative; the predicate was corrected and covered by tests. These runs prove lifecycle cleanup and preserve the blocker; they do not prove T05 green.
...
- **Open blocker/evidence:** B-003 remains open as preserved shell/previous-Harness evidence. The known upstream `Error inside ProxyWorker` regression ([workers-sdk #15317](https://github.com/cloudflare/workers-sdk/issues/15317)) and available remedy ([workers-sdk #15448](https://github.com/cloudflare/workers-sdk/pull/15448)) remain part of the diagnostic history; the Canonical migration must now classify fresh failures under Task 4 before deciding whether the blocker is still external. No preview, vendored, patched, restart, retry, or lowered-expectation workaround is authorized.
- **Next safe action:** Execute Task 2 of the owner-supplied Canonical Harness plan; T06 remains T05-gated for final validation, `STACK_GREEN`, and merge.
---
...
| B-003 | Runtime | Phase F | Full Programs/Worker/D1 journey is unreliable | T05 / #510 | `OPEN` | Deterministic attempts `20260903t203931257z`, `20260903t204826297z`, and `20260903t223541671z` complete build/bundle/migration/direct seed/readiness/demo seed before the unfiltered journey; the latest run reproduces the first-cause `Error in ProxyController` / `Network connection lost.` with Worker exit code 1 and 168 downstream `ERR_CONNECTION_REFUSED` signals. Harness qualification runs `20260903t221201229z`, `20260903t222013107z`, and `20260903t230214275z` also fail required journey rows. Artifacts are preserved under `test-results/programs-d1-runs/<run-id>/` and `test-results/test-harness-qualification/<run-id>/`. Wrangler 4.129.0 is the latest stable checked but does not contain the still-open [workers-sdk #15448](https://github.com/cloudflare/workers-sdk/pull/15448) fix; the regression is tracked in [#15317](https://github.com/cloudflare/workers-sdk/issues/15317). This evidence remains Diagnostic/upstream history while the Canonical migration is executed; Task 4 must classify fresh Canonical failures before B-003 can be resolved. Do not mark `STACK_GREEN` or hide a required row with isolated retries. |
...
1. Execute T05 Task 2: rename and make the official Harness runner testable; keep `scripts/run-programs-acceptance.ts` as Diagnostic.
2. Preserve B-003 evidence and keep T06 final browser validation, `STACK_GREEN`, and merge gated by T05; do not start T07 or Phase 1.
