...
| Programs browser Harness | The runtime runner creates a local Wrangler TestHarness, applies migrations to DB, and serves an ephemeral loopback origin. | Disposable local test harness; the Home acceptance report identifies a localhost port. | Run browser acceptance and seed only its local fixture DB. |
