...
`f6f1d01d5bd658fc62a7a87e5e77bcd3814ba471` (the current participant and
management tracer implementation, fixture-boundary and cleanup hardening,
and the participant detail back-control assertion; the acceptance trace
remains documentation-only on top of it).
...
| T12-P01 | Disposable local D1 contains a unique listed `MemberRequest` Program | A real member logs in and opens ordinary `/programs` | Authenticated Programs directory renders from the real Worker/D1 journey | `test-results/programs-promotion/20260910t071900457z/browser-results.json`, 2/2 | `PASS` |
| T12-P02 | The member's real `/api/v1/programs/access` projection has no management capability | Directory settles | `進入管理模式` is absent; no fake Storybook prop can grant it | Participant response assertion in `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-P03 | Participant directory contains the disposable Program | Member searches a unique fixture name | Search returns the disposable row | `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-P04 | Directory has a non-matching query | Member enters a guaranteed impossible query | No-result state is visible and the disposable row is not visible | `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-P05 | Search is non-empty and no-result state is active | Member clears search/filter | Catalog returns to a usable result state; filter `可報名` is observable and pressed state is truthful | `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-P06 | Disposable Program is listed and eligible | Member selects the row under `可報名`, then activates the detail back control | Canonical participant transition reaches `/programs?program=<id>&from=programs`, visible `#program-detail-title` renders, and the back control returns to ordinary `/programs` before reopening the same Program | Exact href/URL/title/back assertions in `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-P07 | Detail enrollment is `MemberRequest` | Member requests, admin approves, and member reloads | Existing request, approval, read-back, and cancellation journey remains green | `test-results/programs-promotion/20260910t071900457z/browser-results.json`; POST 201, decision 200, cancellation/read-back pass | `PASS` |
| T12-M01 | Disposable local D1 contains a unique manageable Program | A real admin logs in and opens ordinary `/programs` | Real server projection exposes `進入管理模式` | Management response assertion in `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-M02 | Management entry is visible | Admin clicks it | Canonical URL is `/programs?mode=management`; `管理課程目錄` is visible | URL/title assertions in `test-results/programs-promotion/20260910t071900457z/browser-results.json` | `PASS` |
| T12-M03 | Management directory has the disposable Program | Admin searches its unique name | Search returns and opens the selected Program | `test-results/programs-promotion/20260910t071900457z/browser-results.json`; canonical URL assertion | `PASS` |
| T12-M04 | Existing management settings are reachable | Admin edits and reloads | Settings save and read-back remain green; disposable fixture is archived in cleanup | `test-results/programs-promotion/20260910t071900457z/browser-results.json`; PATCH/read-back/cleanup pass | `PASS` |
...
| T12-R01 | Programs contract and responsive suites remain unchanged in authority | Qualification runs | Worker Contract, responsive, build/index, fast/precommit, Programs verification, and census outcomes are recorded truthfully | Promotion `test-results/programs-promotion/20260910t071900457z/promotion.json` is `functional-passed` at `f6f1d01d5bd658fc62a7a87e5e77bcd3814ba471`; census limitation is separately recorded in T12-R02 | `PASS_WITH_SEPARATE_CENSUS_LIMITATION` |
...
| T12-A01 | Current-main merge base is fixed | Standards and Spec review runs | Review uses actual live-main merge base and resolves every real in-scope finding | `origin/main` resolves to `eee8ba1872fbb53ffd835063b78e824b8d187e68`; final two-axis review of the implementation candidate diff found no unresolved hard in-scope finding | `PASS` |
...
`/private/tmp/efcc-t12-storybook-static-f6f1d01d` after verification so it
does not become a worktree artifact.

| Command | Result | Evidence |
|---|---|---|
| `pnpm test:t12:programs-storybook` | PASS, 21/21 | `tests/e2e/test-results/t12-programs-storybook/storybook.json`; exact W7, 1 worker, 0 retries; fresh run started `2026-09-10T07:21:43.022Z` |
| `pnpm test:programs:browser` | PASS, 2/2 | `test-results/programs-promotion/20260910t071900457z/browser-results.json`; target loopback origin, Chromium `phone-390`, 1 worker, 0 retries |
| `pnpm test:programs:contract` | PASS, 1/1 | Promotion records `test-results/programs-promotion/20260910t071900457z/worker-contract.log` |
| `pnpm test:programs:responsive` | PASS, 6/6 | `test-results/programs-promotion/20260910t071900457z/responsive-results.json`; 320/390/1280 matrix, 1 worker, 0 retries |
| `pnpm --dir web storybook:build` | PASS | Storybook 10.6.0 build completed; generated output moved to `/private/tmp/efcc-t12-storybook-static-f6f1d01d` |
...
| `pnpm verify:programs` | PASS, `functional-passed` | `test-results/programs-promotion/20260910t071900457z/promotion.json`; all four finite stages passed at revision `f6f1d01d5bd658fc62a7a87e5e77bcd3814ba471`; B-003 remains OPEN/not run |
...
An earlier clean promotion attempt also captured a single Miniflare
...
`20260910t071900457z` reproduced neither the response failure nor the UI
...
- **Standards:** PASS. No hard documented-standard violation remains; the
  ...
  partial-fixture sentinels. The final delta review at `f6f1d01d` found no new
  standards issue.
- **Spec:** PASS. The participant and management journeys prove the required
  ...
  search/no-result/filter/detail transition, activated detail back control,
  existing enrollment/settings mutations, and cleanup. The W7 runner
  qualifies only the three existing production baselines. The census
  limitation is disclosed rather than hidden, and no T13–T16 scope or fake
  permission authority is introduced. The prior Spec 392 ORG-02 P2 is closed
  by the fresh back-control assertion and final promotion run.
...
- **Commits:** `882c4c32` Ticket 00 contract; `a0029045` participant tracer;
  ...
  hardening; `de287156` disposable fixture closure; `f6f1d01d` participant
  detail back-control contract. The final trace update is documentation-only
  on top of this implementation candidate.
