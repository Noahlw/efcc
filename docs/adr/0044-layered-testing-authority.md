...
This amendment supersedes only the former mandatory five-minute promotion prerequisite. It does not remove coverage, change baselines or tolerances, or replace the real Worker/D1 browser seam. At the time, it did not authorize another runtime adapter; the later amendment below narrows that process choice after the configured D1 setup stalled. B-003 is re-evaluated at ordinary finite failures, relevant Worker/D1/Harness/toolchain changes, T33/#538 reconciliation, and before T35/#540 final owner release approval.

## Amendment — local E2E D1 setup boundary

On 2026-09-24, the Browser Acceptance, Responsive Matrix, and Runtime Reliability Canary runners stopped at host-side D1 setup through `createTestHarness()` (`applyD1Migrations()` / `getEnv()`) with the repository's D1 binding and pinned Wrangler 4.131.2; the Worker HTTP boundary itself remained reachable. Cloudflare documents those methods for harness test-state setup, but the calls stalled in this observed configuration before fixture seeding. The harness also uses non-persistent storage, so CLI-seeded Wrangler local D1 would not be the same database.

The real Worker/D1 acceptance boundary remains unchanged. These end-to-end runners now start the configured Worker with `wrangler dev --local`, apply its checked-in migrations and E2E SQL fixtures through `wrangler d1 ... --local --persist-to <per-run-temp-directory>`, and use that same directory for the Worker process. The temporary D1 is removed on runner shutdown. This uses Cloudflare's local Worker/D1 runtime, not an application mock or alternate product runtime; no remote resource is selected. This amendment supersedes only `createTestHarness()` as the process/setup mechanism for these end-to-end runners. Worker contract tests may continue using the Workers Vitest integration, and `createTestHarness()` remains suitable for integration tests where its required binding APIs complete. Scenario ownership, behavior assertions, report requirements, and historical-suite contraction gates remain unchanged.
