...
- **Status**: Proposed — decision locked via grilling of issue [#128](https://github.com/Noahlw/efcc/issues/128). The HTTP client (`web/lib/api.ts`) and same-origin proxy Worker (`web/worker.ts`) are downstream CF0 implementation evidence for [#142](https://github.com/Noahlw/efcc/issues/142) and the throwaway dispatcher in `src/gas/prototype-129-http-dispatch.gs` is downstream prototype evidence for [#129](https://github.com/Noahlw/efcc/issues/129); none of these are work or acceptance claims for #128 itself. The ADR remains `Proposed` under `AGENTS.md` until the CF0 implementation ticket and the [#131](https://github.com/Noahlw/efcc/issues/131) server-side dispatcher are both accepted; flip to Accepted then, with the implementation ticket, deployment version, date, and observed result appended here.
...
- `X-Efcc-Session-Id: <sessionId>` — the public lookup key. Required for `restoreApp`, `logoutUser`, `authorizedNavigate`, and every other protected action, because Apps Script `doPost` cannot read request headers (verified against `developers.google.com/apps-script/guides/web`) and the server must resolve `sessionId` → `userId` via `sessionLookup_`.

`loginUser` sends no session headers (anonymous by definition). All other actions send both. The Worker forwards both headers untouched to Apps Script on the upstream `fetch`; it never rewrites, decodes, or echoes them back.

The HTTP client (`web/lib/api.ts` `callRpc`) builds both headers from the typed `Session` object passed in; the Worker (`web/worker.ts`) reads them and forwards them; the server-side dispatcher (§10) validates the bearer against the sessionId lookup result and rejects with `AUTH_REQUIRED` on mismatch. The current throwaway dispatcher in `src/gas/prototype-129-http-dispatch.gs` may use non-secret `userId`/`sessionId` body fields as interim compatibility because Apps Script cannot see request headers; it must never receive or trust `sessionToken` in `body.params`. The production dispatcher must derive the actor from the verified session lookup, and the interim path is not acceptance evidence for this ADR.
...
- The Worker forwards only `Content-Type`, `Authorization`, `X-Efcc-Session-Id`, and `Idempotency-Key` to the upstream; the browser `Origin` is **not** forwarded (server-side `fetch` is not subject to CORS, and Apps Script never needs to see a browser `Origin`).
...
Adopted near-standard `Idempotency-Key` (IETF `Idempotency-Key` HTTP header draft, MDN-documented) generalized from the existing bespoke `requestKey` field. The client attaches it automatically for any action in the `MUTATING_ACTIONS` set (`web/lib/api.ts`):
...
The client generates the key with `crypto.randomUUID()` (available in all modern browsers and in `workerd`) unless the caller passes one explicitly via `options.idempotencyKey`. The Worker forwards `Idempotency-Key` to Apps Script on the upstream `fetch`. The server-side behavior on dedup is the [#131](https://github.com/Noahlw/efcc/issues/131) ticket's decision; the wire contract here is only that the header is present on mutating calls and absent on reads.
...
Limits themselves are coarse — a tripwire against Apps Script's 30-simultaneous-execution quota, the one "Watch" item in the cost model. Exact thresholds belong to the CF0 implementation ticket, not this ADR. When the binding is missing (dev / test), the Worker skips limiting rather than blocking unidentified traffic; this is intentional and matches the binding's optional typing (`RPC_RATE_LIMITER?: RateLimit`).
...
6. The Worker forwards only `Content-Type`, `Authorization`, `X-Efcc-Session-Id`, and `Idempotency-Key` to `APPS_SCRIPT_EXEC_URL`. The browser `Origin` is never forwarded.
...
- **Session token in URL or body — rejected.** The **Session Transport** rule in `AGENTS.md` forbids credentials in URLs and query strings; this ADR extends the same trust-boundary rule to action body parameters. `localStorage` already gives the browser a typed `Session` object from which to build the header. Header transport is the only option that survives a leaked referrer, a shared screenshot, or a future server log.
