...
3. Focused component contract tests (`web/lib/app.test.tsx`) still pass — the default signed-out login form's observable contract is preserved.
...
| L3 | Default login form present and labeled | `getByLabelText("用戶名稱")` and `getByLabelText("PIN 碼")` inputs exist on cold boot; one submit button named `登入` |
...
- No changes to the existing default `COPY.login.*` values; upgrade-specific keys are governed by Spec 077.
...
docs/specs/076-landing-page-acceptance-plan.md). Forced credential upgrade is
not part of this plan's acceptance boundary; see Spec 077.