...
| H-07 | The operator renames a Role Definition whose name collides with an existing Role Definition name | Submit the rename | `409` Problem Details with `code=ROLE_NAME_TAKEN`; no row is written; no revision advances; a REJECTED audit row is recorded. |
...
| H-13 | The operator renames with an idempotency key already used for a different change | Submit the rename | `409` Problem Details with `code=ROLE_IDEMPOTENCY_REUSE`; no mutation; a REJECTED audit row is recorded. |
