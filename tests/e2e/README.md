...
## Required single-process Programs runtime gate

After creating `web/.dev.vars`, run the complete disposable local prerequisite with one Worker process:

```sh
pnpm verify:programs-runtime
```

The runner executes one explicit sequence: build the static export, bundle the Worker, apply local migrations with `--local --persist-to <run>/wrangler-state`, direct-seed disposable identity data with the same persistence path while no Worker is serving, start exactly one long-lived loopback Worker on the selected local port with the same persistence path, wait for listener readiness, perform a real disposable-admin login and authenticated `/api/v1/auth/me` probe, seed the demo/domain state through the real Worker API, and execute the unfiltered `tests/e2e/programs-d1.config.ts` journey with retries disabled. Preparation and post-start child stages have bounded timeouts; SIGINT/SIGTERM use the same process-group cleanup path. It exits non-zero when the Worker or any required Playwright row fails. Every invocation gets a new timestamped directory under `test-results/programs-d1-runs/` with `run.json`, `prepare.log`, `worker.log`, `wrangler.log`, `seed.log`, `playwright.log`, the built-in Playwright JSON result, `wrangler-state/`, and `failure-summary.json`; later clean runs never overwrite a failed stale-fixture run. The failure summary keeps the first ProxyController/workerd marker separate from downstream connection refusals or resets.

The optional `pnpm qualify:test-harness` command is a bounded diagnostic for Wrangler's official `createTestHarness()` API. It uses a fresh harness, the built Worker, real local D1 migrations/fixtures, the real demo API seed, and the same unfiltered journey, then preserves its artifacts under `test-results/test-harness-qualification/`. It is not the canonical acceptance path and cannot replace the shell runner unless repeated equivalent, reliable results are recorded in the Phase 0 tracker.
...
Geometry reports live under `tests/e2e/test-results/phase-f/<suite>/results.json`. Route-only reports remain at their suite-specific paths such as `tests/e2e/test-results/auth-d1-results.json`, `live-ui-results.json`, and `attendance-d1-results.json` when those suites pass. The required single-process `programs-d1` journey writes the built-in JSON report and raw Worker/seed/Playwright logs under the unique `test-results/programs-d1-runs/<run-id>/` directory; failed runs remain reviewable and a missing or failed report is not evidence. The rendered JSON/HTML, the acceptance trace, `docs/qa/2026-09-01-s4-phase-f-release-gate.md`, and `docs/qa/2026-09-01-s4-phase-f-audit-dispositions.md` are the reviewable outputs. Numeric evidence proves DOM/API geometry and state only; it does not claim human keyboard/AT, real-device camera/touch, native print-preview, forced-colors, zoom/reflow, or text-spacing outcomes.
