...
4. Capture Google sessions for all three roles, signing in with the correct Google account each time:
   ...
   ```
5. Run the suite: `npm run test:e2e`.
...
`tests/e2e/playwright.config.ts` defines the `alice`, `bob`, and `noah` projects and loads their corresponding `.auth/*.storage.json` files. The suite runs sequentially with one worker, and the JSON reporter writes `test-results/e2e-results.json`. For local convenience, a successful `npm run test:e2e` invokes the `posttest:e2e` npm hook, which runs `plan-doc-appender.ts` and upserts an `## Executed results` section into the default plan doc `docs/specs/067-role-nav-acceptance-plan.md`. npm skips the post-hook when the main script fails. CI therefore invokes `npx playwright test --config=tests/e2e/playwright.config.ts` directly (without the npm post-hook), then runs the appender in a separate `if: always() && hashFiles('test-results/e2e-results.json') != ''` step. If you invoke `npx playwright test --config=tests/e2e/playwright.config.ts` manually outside CI, the post-hook is also bypassed; run `npx tsx tests/e2e/plan-doc-appender.ts` yourself to update the default plan doc, or add `--plan <other-path>` to target another plan doc.
...
CI fails loudly rather than skipping an affected role. Re-run `npm run e2e:auth -- --role=<role>` for that role, confirm the local run, base64-encode the replacement file, and update the matching repository secret. Do not print or paste decoded storage-state JSON into logs or issues.
...
`.github/workflows/e2e.yml` runs on every push using Node 20. It installs dependencies with `npm ci`, installs Chromium with system dependencies, decodes the three secrets into `.auth/*.storage.json`, and invokes `npx playwright test --config=tests/e2e/playwright.config.ts` with the repository variable `E2E_TARGET_URL`. Unlike the local `npm run test:e2e` path, this direct `npx` invocation does not trigger the npm post-hook or double-fire the appender.
