...
test.describe("PUI-04 participant Enrollment lifecycle", () => {
  test("member can submit one request, see Pending, and withdraw it", async ({
    ...
    await enrollmentPanel
      ...
      .click();
    await expect(
      enrollmentPanel.getByText(COPY.confirmWithdrawRequest)
    ).toBeVisible();
    await enrollmentPanel
...
test.describe("MUI-01 management Directory and Workspace", () => {
  test("admin searches the scoped Directory and navigates focused Workspace tasks", async ({
    ...
    await page.getByRole("link", { name: COPY.workspaceTitle }).click();
    await expect(page).toHaveURL(
      new RegExp(`/programs\\?mode=management&program=${programId}$`, "u")
    );
    await page
...
test.describe("AUTH-01 Program Leader administration", () => {
  test("Staff denies self-assignment, revokes the seeded leader, and re-grants it", async ({
    ...
    await expect(leadersPanel.getByText(/E2E Member/u).first()).toBeVisible();
    ...
    try {
      ...
      await leadersPanel.getByRole("option", { name: /E2E Staff/u }).click();
      await leadersPanel
      ...
      await leadersPanel.getByRole("option", { name: /E2E Member/u }).click();
      await leadersPanel
        .getByRole("button", { name: COPY.assignLeader })
        .click();
      await expect(
  ...
  test("Admin grants a Department Manager, scope inherits, then revokes", async ({
    ...
    try {
      ...
      await page
        ...
        .click();
      await page
        .getByRole("button", { name: COPY.departmentManagers, exact: true })
        .click();

      const managersPanel = page.getByRole("region", {
        name: new RegExp(`${COPY.departmentManagers}.*E2E_DEMO_示範事工`, "u"),
      });
      ...
      await combo.click();
      await combo.fill("E2E_member");
      await managersPanel.getByRole("option", { name: /E2E Member/u }).click();
      await managersPanel
      ...
      await expect(
        managersPanel.getByText(/E2E Member/u).first()
      ).toBeVisible();
      ...
      for (const programName of [
        ...
        await expect(
          page.getByRole("button", { name: new RegExp(programName, "u") })
        ).toBeVisible();
      }
      await expect(
        page.getByRole("button", { name: /E2E_DEMO_示範事工.*部門設定/u })
      ).toBeVisible();
      ...
      await page
        .getByRole("button", { name: COPY.departmentManagers, exact: true })
        .click();
      const managersPanel2 = page.getByRole("region", {
        name: new RegExp(`${COPY.departmentManagers}.*E2E_DEMO_示範事工`, "u"),
      });
      await expect(
        managersPanel2.getByText(/E2E Member/u).first()
      ).toBeVisible();
...
test.describe("EVT-01 event operational detail and availability", () => {
  ...
  test("admin creates, deep-links, and edits an event with HK wall display", async ({
    ...
    await expect(
      page.getByText(COPY.eventManualSource, { exact: true })
    ).toBeVisible();
    await expect(
...
test.describe("EVT-02 recurring preview and generation", () => {
  ...
  async function openScheduleTask(
    ...
    await expect(
      page.getByRole("heading", { name: COPY.settingsSchedule, exact: true })
    ).toBeVisible();
  }
  ...
  test("generation reports deterministic created/skipped counts and refreshes the directory", async ({
    ...
    await expect.poll(() => eventCount(page, id)).toBeGreaterThan(before);
