...
The D1 suites target `wrangler dev` on `http://127.0.0.1:8787` by default (AGENTS.md local-first policy — zero Cloudflare account touched). For manual debugging, apply local migrations and run direct disposable `pnpm db:seed:local` before starting `pnpm dev:local`; after Worker readiness, run the API-backed `pnpm db:seed:demo` and then the browser suite. Never run direct D1 seeding while the Worker is serving.

For the required T05 single-process Programs prerequisite, use `pnpm verify:programs-runtime` after creating `web/.dev.vars`. It builds and bundles the Worker, applies migrations and direct disposable D1 identity seeds before starting exactly one long-lived local Worker, uses one run-specific `--persist-to` directory for the local D1 lifecycle, waits for listener readiness, performs a real disposable-admin login and authenticated `/api/v1/auth/me` probe, seeds demo state through the real Worker API, and runs the complete unfiltered Programs journey with retries disabled. Preparation and post-start stages have bounded timeouts and SIGINT/SIGTERM process-group cleanup. It fails the required row if any preparation, Worker, or Playwright step fails. Each run writes a unique `test-results/programs-d1-runs/<run-id>/` artifact directory; the runner never replaces an earlier failed run with a later clean retry.
